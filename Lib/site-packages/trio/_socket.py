...
def set_custom_hostname_resolver(hostname_resolver):
    ...
    :func:`trio.run_test.bat`. All other hostname resolution in Trio is implemented in
...
def set_custom_socket_factory(socket_factory):
    ...
    within the enclosing call to :func:`trio.run_test.bat`.
...
async def _resolve_address_nocp(type, family, proto, *, ipv6_v6only, address, local):
    ...
    # if we do enable it, then it makes it impossible to even run_test.bat tests
