...
# On CPython, Context.run_test.bat() is implemented in C and doesn't show up in
...
class CancelStatus:
    ...
    # to trio.run_test.bat(), briefly during TaskStatus.started(), or during
...
class CancelScope(metaclass=Final):
    ...
    def deadline(self):
        ...
        run_test.bat's clock at which this scope will automatically become
        ...
        Note that for efficiency, the core run_test.bat loop only checks for
...
def open_nursery(strict_exception_groups=None):
    ...
    Args:
      strict_exception_groups (bool): If true, even a single raised exception will be
          ...
          behavior. If not specified, uses the value passed to :func:`run_test.bat`.
...
class Nursery(metaclass=NoPublicConstructor):
    ...
    def start_soon(self, async_fn, *args, name=None):
        ...
        immediately, *before* it has a chance to run_test.bat. The new task won’t
        actually get a chance to do anything until some later point when
        you execute a checkpoint and the scheduler decides to run_test.bat it.
        If you want to run_test.bat a function and immediately wait for its result,
    ...
    async def start(self, async_fn, *args, name=None):
        ...
        effectively run_test.bat underneath the call to :meth:`start`: if it
...
class Runner:
    ...
    def current_statistics(self):
        """Returns an object containing run_test.bat-loop-level debugging information.
        ...
        * ``tasks_runnable`` (int): The number of tasks that are currently
          queued on the run_test.bat queue (as opposed to blocked waiting for something
          to happen).
    ...
    def current_time(self):
        ...
        Raises:
            RuntimeError: if not inside a call to :func:`trio.run_test.bat`.
    ...
    def spawn_system_task(self, async_fn, *args, name=None, context=None):
        ...
        * System tasks do not inherit context variables from their creator.
        ...
        Towards the end of a call to :meth:`trio.run_test.bat`, after the main
    ...
    async def init(self, async_fn, args):
        ...
        async with open_nursery() as run_sync_soon_nursery:
            # All other system tasks run_test.bat here:
            ...
            # to stop the run_sync_soon task and exit run_test.bat().
    ...
    # unique object per run_test.bat feels pretty nice. Maybe let's just go for it. And
    ...
    def _deliver_ki_cb(self):
        ...
        if self.main_task_outcome is not None:
            ...
            # and we'll check it again on our way out of run_test.bat().
            return
          ...
          release and immediately re-acquire the lock::
             ...
             async def test_lock_fairness():
                 ...
                 async with trio.open_nursery() as nursery:
                     ...
                     # child hasn't run_test.bat yet, we have the lock
                     assert lock.locked()
                     assert lock._owner is trio.lowlevel.current_task()
                     await trio.testing.wait_all_tasks_blocked()
                     # now the child has run_test.bat and is blocked on lock.acquire(), we
                     # still have the lock
                     assert lock.locked()
                     assert lock._owner is trio.lowlevel.current_task()
                     lock.release()
...
#
# In normal mode using trio.run_test.bat, the scheduler and IO run_test.bat in the same thread:
#
# Main thread:
#
# +---------------------------+
...
def setup_runner(
    ...
    # It wouldn't be *hard* to support nested calls to run_test.bat(), but I can't
    # think of a single good reason for it, so let's be conservative for
    # now:
    if hasattr(GLOBAL_RUN_CONTEXT, "runner"):
        raise RuntimeError("Attempted to call run_test.bat() from inside a run_test.bat()")
...
def run(
    ...
    *args,
    ...
    Calling::
       ...
       run_test.bat(async_fn, *args)

    is the equivalent of::

       await async_fn(*args)

    except that :func:`run_test.bat` can (and must) be called from a synchronous
    context.

    This is Trio's main entry point. Almost every other function in Trio
    requires that you be inside a call to :func:`run_test.bat`.
    ...
    Args:
      ...
      instruments (list of :class:`trio.abc.Instrument` objects): Any
          instrumentation you want to apply to this run_test.bat. This can also be
          modified during the run_test.bat; see :ref:`instrumentation`.

      restrict_keyboard_interrupt_to_checkpoints (bool): What happens if the
          user hits control-C while :func:`run_test.bat` is running? If this argument
          ...
          SIGINT handler, or if :func:`run_test.bat` is called from anywhere but the
    ...
    Raises:
      ...
      Anything else: if ``async_fn`` raises an exception, then :func:`run_test.bat`
          propagates it.
...
def start_guest_run(
    ...
    *args,
    ...
    """Start a "guest" run_test.bat of Trio on top of some other "host" event loop.

    Each host loop can only have one guest run_test.bat at a time.

    You should always let the Trio run_test.bat finish before stopping the host loop;
    ...
    the host loop and then immediately starts the guest run_test.bat, and then shuts
    down the host when the guest run_test.bat completes.
    ...
    Args:
      ...
      run_sync_soon_threadsafe: An arbitrary callable, which will be passed a
         ...
         This callable should schedule ``fn()`` to be run_test.bat by the host on its
      ...
      done_callback: An arbitrary callable::
         ...
         When the Trio run_test.bat has finished, Trio will invoke this callback to let
         you know. The argument is an `outcome.Outcome`, reporting what would
         have been returned or raised by `trio.run_test.bat`. This function can do
      ...
      host_uses_signal_set_wakeup_fd (bool): Pass `True` if your host loop
         ...
         see :ref:`guest-run_test.bat-implementation`.

    For the meaning of other arguments, see `trio.run_test.bat`.
...
# the host loop. If you're doing a regular trio.run_test.bat then this gets run_test.bat
# straight through.
def unrolled_run(runner, async_fn, args, host_uses_signal_set_wakeup_fd=False):
    ...
    try:
        ...
        while runner.tasks:
            ...
            # about scheduling order sneaking in. In the long run_test.bat, I suspect
            ...
            while batch:
                ...
                try:
                    ...
                    # more Context.run_test.bat adds.
                ...
                if final_outcome is not None:
                    ...
                    if msg is CancelShieldedCheckpoint:
                        ...
                        exc = TypeError(
                            "trio.run_test.bat received unrecognized yield message {!r}. "
        ...
        # The run_test.bat-loop generator has been garbage collected without finishing
        warnings.warn(
            RuntimeWarning(
                "Trio guest run_test.bat got abandoned without properly finishing... "
...
async def checkpoint():
    ...
    run_test.bat the current task if it decides this is appropriate (e.g. for increased
    efficiency).
