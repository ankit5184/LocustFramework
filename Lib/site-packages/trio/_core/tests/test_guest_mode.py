...
# Nice features:
# - we can run_test.bat code "outside" of trio using the schedule function passed to
#   our main
...
def trivial_guest_run(trio_fn, **start_guest_run_kwargs):
    ...
    def run_sync_soon_threadsafe(fn):
        if host_thread is threading.current_thread():  # pragma: no cover
            ...
            todo.put(("run_test.bat", crash))
        todo.put(("run_test.bat", fn))
    ...
    try:
        ...
        # if an exception does cause us to abandon a run_test.bat then the Trio state
    ...
    async def trio_main(in_host):
        ...
        async def get_woken_by_host_deadline(watb_cscope):
            with trio.CancelScope() as cscope:
                ...
                # future, will cause the run_test.bat loop to wake up, but then
...
def test_guest_warns_if_abandoned():
    ...
    # sure that our abandoned run_test.bat is garbage. The easiest way to do this is to
    ...
    with pytest.warns(RuntimeWarning, match="Trio guest run_test.bat got abandoned"):
...
def test_guest_mode_asyncgens():
    ...
    # Ensure we don't pollute the thread-level context if run_test.bat under
