...
def test_asyncgen_basics():
    ...
    async def async_main():
        ...
        if buggy_pypy_asyncgens:
            collected.append("outlived run_test.bat")
        else:
            saved.append(example("outlived run_test.bat"))
            assert 42 == await saved[-1].asend(None)
            assert collected == []

    _core.run(async_main)
    assert collected.pop() == "outlived run_test.bat"
    for agen in saved:
...
async def step_outside_async_context(aiter):
    # abort_fns run_test.bat outside of task context, at least if they're
    ...
    # reference to aiter inside the guts of the run_test.bat loop.
    def abort_fn(_):
