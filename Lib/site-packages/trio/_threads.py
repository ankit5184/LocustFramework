...
async def to_thread_run_sync(sync_fn, *args, cancellable=False, limiter=None):
    ...
    * If ``cancellable=True``, then this function immediately raises
      ...
      function has returned. If :func:`trio.run_test.bat` finishes before the thread
    ...
    # This function gets scheduled into the Trio run_test.bat loop to deliver the
    # thread's result.
    ...
    def deliver_worker_fn_result(result):
        try:
            ...
            # The entire run_test.bat finished, so the task we're trying to contact is
...
def _run_fn_as_system_task(cb, fn, *args, context, trio_token=None):
    """Helper function for from_thread.run_test.bat and from_thread.run_sync.
...
def from_thread_run(afn, *args, trio_token=None):
    ...
    Raises:
        RunFinishedError: if the corresponding call to :func:`trio.run_test.bat` has
            already completed, or if the run_test.bat has started its final cleanup phase
            and can no longer spawn new system tasks.
        Cancelled: if the corresponding call to :func:`trio.run_test.bat` completes
    ...
    **Locating a Trio Token**: There are two ways to specify which
    `trio.run_test.bat` loop to reenter:
        ...
        - Pass a keyword argument, ``trio_token`` specifying a specific
          `trio.run_test.bat` loop to re-enter. This is useful in case you have a
...
def from_thread_run_sync(fn, *args, trio_token=None):
    ...
    Raises:
        RunFinishedError: if the corresponding call to `trio.run_test.bat` has
            already completed.
        RuntimeError: if you try calling this from inside the Trio thread,
            which would otherwise cause a deadlock.
        AttributeError: if no ``trio_token`` was provided, and we can't infer
