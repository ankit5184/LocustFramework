...
def compatible_platforms(provided, required):
    """Can code for the `provided` platform run_test.bat on the `required` platform?
...
def run_script(dist_spec, script_name):
    """Locate distribution `dist_spec` and run_test.bat its `script_name` script"""
...
class WorkingSet:
    ...
    def run_script(self, requires, script_name):
        """Locate distribution for `requires` and run_test.bat `script_name` script"""
        ns = sys._getframe(1).f_globals
        name = ns['__name__']
        ns.clear()
        ns['__name__'] = name
