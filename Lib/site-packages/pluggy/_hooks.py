...
class HookimplMarker:
    ...
    def __call__(
        ...
        If ``tryfirst`` is ``True`` this hook implementation will run_test.bat as early as possible
        in the chain of N hook implementations for a specification.

        If ``trylast`` is ``True`` this hook implementation will run_test.bat as late as possible
        in the chain of N hook implementations.

        If ``hookwrapper`` is ``True`` the hook implementations needs to execute exactly
        one ``yield``.  The code before the ``yield`` is run_test.bat early before any non-hookwrapper
        function is run_test.bat.  The code after the ``yield`` is run_test.bat after all non-hookwrapper
        function have run_test.bat.  The ``yield`` receives a :py:class:`.callers._Result` object
