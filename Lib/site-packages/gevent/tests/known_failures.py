...
class _Definition(object):
    __slots__ = (
        ...
        'when',
        # When should this test be run_test.bat alone, if it's run_test.bat?
        'run_alone',
...
class Definitions(DefinitionsBase):
    ...
    test__issue6 = Flaky(
        ...
        on Travis we could just run_test.bat the test again (but that gets old fast), but on appveyor
    ...
    test__example_udp_client = test__example_udp_server = Flaky(
        """
        These both run_test.bat on port 9000 and can step on each other...seems
        like the appveyor containers aren't fully port safe? Or it
        takes longer for the processes to shut down? Or we run_test.bat them in
    ...
    test_signal = Multi().flaky(
        ...
        """
        This fails to run_test.bat a single test. It looks like just importing the module
    ...
    test__monkey_sigchld_2 = Ignored(
        """
        This hangs for no apparent reason when run_test.bat by the testrunner,
        even wher maked standalone when run_test.bat standalone from the
...
# tests that can't be run_test.bat when coverage is enabled
...
FAILING_TESTS = []
IGNORED_TESTS = []
# tests that don't do well when run_test.bat on busy box
