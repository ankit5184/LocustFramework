...
class TestSemaphoreMultiThread(greentest.TestCase):
    ...
    def _do_test_acquire_in_one_then_another(self,
        ...
        if release:
            ...
            # (Release schedules the notifier to run_test.bat, and when the
            # notifier run_test.bat it sends the async notification to the
            # other thread. Depending on exactly where we are in the
            # event loop, and the limit to the number of callbacks
            # that get run_test.bat (including time-based) the notifier may or
            # may not be immediately ready to run_test.bat, so this can take up
            # to two iterations.)
