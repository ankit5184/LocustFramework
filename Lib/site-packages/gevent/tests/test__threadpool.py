...
class TestPool(_AbstractPoolTest):
    ...
    def test_async_callback(self):
        ...
        self.assertEqual(get(), 49)
        self.assertTimeoutAlmostEqual(get.elapsed, TIMEOUT1, 1)
        gevent.sleep(0)  # lets the callback run_test.bat
...
class TestJoinEmpty(TestCase):
    ...
    # Running this test standalone doesn't crash PyPy, only when it's run_test.bat
...
class TestTPE(_AbstractPoolTest):
    ...
    def test_future(self):
        ...
        # greenlet we spawned got to run_test.bat while we waited.

        self.assertEqual(future.result(), 42)
        self.assertTrue(future.done())
        self.assertFalse(future.cancelled())
        # Make sure the notifier has a chance to run_test.bat so the call back
        # gets called
        gevent.sleep()
...
class TestThreadResult(greentest.TestCase):
    ...
    def test_exception_in_on_async_doesnt_crash(self):
        ...
        # Spin the loop a few times to make sure we run_test.bat the callbacks.
