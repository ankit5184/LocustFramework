...
Sometimes it is useful to run_test.bat existing python scripts or modules that
were not built to be gevent aware under gevent. To do so, this module
can be run_test.bat as the main module, passing the script and its arguments.
...
def patch_os():
    ...
    .. caution:: For `SIGCHLD` handling to work correctly, the event loop must run_test.bat.
...
to run_test.bat a module instead, pass the `--module` argument before the module name.
