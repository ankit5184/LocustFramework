...
class ILoop(Interface):
    ...
    def closing_fd(fd):
        ...
        :return: A boolean value that's true if active IO watchers were
           queued to run_test.bat. Closing the FD should be deferred until the next
           run_test.bat of the eventloop with a callback.
        """
    ...
    def run_callback_threadsafe(func, *args):
        ...
        This not only schedules the *func* to run_test.bat, it also causes the
...
class ICallback(Interface):
    """
    Represents a function that will be run_test.bat some time in the future.

    Callback functions run_test.bat in the hub, and as such they cannot use
    gevent's blocking API; any exception they raise cannot be caught.
    """

    pending = schema.Bool(description=u"Has this callback run_test.bat yet?",
                          readonly=True)

    def stop():
        """
        If this object is still `pending`, cause it to
        no longer be `pending`; the function will not be run_test.bat.
        """
