...
class loop(AbstractLoop):
    ...
    def __check_and_die(self):
        if not self.ptr:
            # We've been destroyed during the middle of self.run_test.bat().
            ...
            getcurrent().parent.throw(LoopExit('Destroyed during run_test.bat'))
    ...
    def _run_callbacks(self):
        ...
        # We need to run_test.bat them so that any manual callbacks they want to schedule
        ...
        # callbacks to actually stop the watcher; if those don't run_test.bat before
        # we poll for IO, then libuv can abort the process for the closed file descriptor.

        # XXX: There's still a race condition here because we may not run_test.bat *all* the manual
        ...
        # self.run_test.bat() manually process manual callbacks before
        # continuing solves the problem. (But we must still run_test.bat callbacks
        # here again.)
    ...
    def _start_callback_timer(self):
        # The purpose of the callback timer is to ensure that we run_test.bat
        ...
        # our normal prepare watcher kicks in to run_test.bat the callbacks.

        # In libuv, however, timers are run_test.bat *first*, before prepare
        # callbacks and before polling for IO. So a no-op 0 duration
        # timer actually does *nothing*. (Also note that libev queues all
        # watchers found during IO poll to run_test.bat at the end (I think), while libuv
        ...
        #   ...
        #   uv__io_poll(loop, timeout); # <--- IO watchers run_test.bat here!
        #   uv__run_check(loop);
        ...
        #    run_test.bat callbacks in this order: (although specificying priorities changes it)
        #        check
        #        stat
        #        child
        #        signal
        ...
        # the loop run_test.bat the callbacks, we must explicitly run_test.bat them here.

        # If we don't, test__systemerror:TestCallback will be flaky, failing
        # one time out of ~20, depending on timing.

        # To get them to run_test.bat immediately after this current loop,
        ...
        # expire *and be run_test.bat in the same iteration of the loop*. This
        ...
        # other timers get a chance to run_test.bat before our check or prepare
        # watcher callbacks do. Therefore, we do indeed have to have a 0
        # timer to run_test.bat callbacks---it gets inserted before any other user
    ...
    def run(self, nowait=False, once=False):
        ...
        if mode == libuv.UV_RUN_DEFAULT:
            while self._ptr and self._ptr.data:
                ...
                # It may get run_test.bat again from the prepare watcher, so
                # potentially we could take twice as long as the
                # switch interval.
                # If we have *lots* of callbacks to run_test.bat, we may not actually
                ...
                # Note that we run_test.bat queued callbacks when the prepare watcher runs,
                ...
                if not ran_status and not ran_callbacks and not self._prepare_ran_callbacks:
                    ...
                    # If we didn't run_test.bat any callbacks, then we couldn't schedule
    ...
    def _sigchld_callback(self):
        ...
        # sigchld to run_test.bat when we run_test.bat callbacks.
        while True:
            try:
    ...
    def prepare(self, ref=True, priority=None):
        # We run_test.bat arbitrary code in python_prepare_callback. That could switch
