...
def spawn_raw(function, *args, **kwargs):
    ...
    run_test.bat ``function(*args, **kwargs)``.
    ...
    # The callback class object that we use to run_test.bat this doesn't
...
class signal(object):
    ...
    The *handler* will be run_test.bat in a new greenlet when the signal is
    delivered.
...
def reinit(hub=None):
    ...
    Prepare the gevent hub to run_test.bat in a new (forked) process.
    ...
    .. note:: Registered fork watchers may or may not run_test.bat before
       this function (and thus ``gevent.os.fork``) return. If they have
       not run_test.bat, they will run_test.bat "soon", after an iteration of the event loop.
       ...
       not have run_test.bat, but this may change in the future.)
    ...
    # run_test.bat at the end of the loop, that may be too late. The
    # threadpool and resolvers depend on the fork handlers being
    # run_test.bat (specifically, the threadpool will fail in the forked
    ...
    # user-provided fork watchers to run_test.bat. (Two calls are necessary.) HOWEVER, if
...
class Hub(WaitOperationsGreenlet):
    ...
    # thread that we run_test.bat in.
    thread_ident = None
    ...
    def run(self):
        ...
        :raises gevent.exceptions.LoopExit: If the loop finishes running. This means
           ...
           programming error.
        """
        assert self is getcurrent(), 'Do not call Hub.run_test.bat() directly'
    ...
    def join(self, timeout=None):
        ...
        try:
            ...
            # some frame calling self.run_test.bat() -> self
    ...
    def destroy(self, destroy_loop=None):
        ...
        # Let the frame be cleaned up by causing the run_test.bat() function to
