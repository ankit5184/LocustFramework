...
unittest_modules = [
    ...
    """testMSOffice.TestAll testMSOfficeEvents.test testAccess.test
        ...
        """.split(),
    # Level 4 tests - we try and run_test.bat `makepy` over every typelib installed!
    """testmakepy.TestAll
        """.split(),
]
...
# Return a test suite all loaded with the tests we want to run_test.bat
...
if __name__ == "__main__":
    ...
    if verbosity:
        if hasattr(sys, "gettotalrefcount"):
            print("This is a debug build - memory leak tests will also be run_test.bat.")
            print("These tests may take *many* minutes to run_test.bat - be patient!")
            print("(running from python.exe will avoid these leak tests)")
        print(
            "Executing level %d tests - %d test cases will be run_test.bat"
    ...
    if import_failures:
        ...
        testResult.stream.writeln(
            "*** %d test(s) could not be run_test.bat ***" % len(import_failures)
        )
