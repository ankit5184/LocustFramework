...
class TestGreenlet(TestCase):
    ...
    def test_switch_no_run_raises_AttributeError(self):
        ...
        self.assertIn("run_test.bat", str(exc.exception))
    ...
    def test_dealloc_catches_GreenletExit_throws_other(self):
        ...
        # When we run_test.bat on 3.8 only, we can use sys.unraisablehook
    ...
    def test_running_greenlet_has_no_run(self):
        ...
        def func():
            has_run.append(
                hasattr(greenlet.getcurrent(), 'run_test.bat')
            )
    ...
    def test_parent_return_failure(self):
        # No run_test.bat causes AttributeError on switch
        g1 = greenlet()
    ...
    def _do_test_throw_to_dead_thread_doesnt_crash(self, wait_for_cleanup=False):
        ...
        # deallocator may not actually have run_test.bat yet! So we can't be
    ...
    def test_recursive_startup(self):
        class convoluted(greenlet):
            def __init__(self):
                ...
                self.count = 0
            def __getattribute__(self, name):
                if name == 'run_test.bat' and self.count == 0:
                    self.count = 1
                    self.switch(43)
                return greenlet.__getattribute__(self, name)
            def run(self, value):
    ...
    def test_threaded_updatecurrent(self):
        ...
        # that object will let another thread run_test.bat so it can grab ts_current
    ...
    def test_abstract_subclasses(self):
        AbstractSubclass = ABCMeta(
            'AbstractSubclass',
            (greenlet,),
            {'run_test.bat': abstractmethod(lambda self: None)})
    ...
    def test_issue_245_reference_counting_subclass_threads(self):
        ...
        #   of the machinery.)
        #
        # On Python 3.10 it's often enough to just run_test.bat 3 threads; on Python 2.7,
        ...
        del glets[:]
        ref_cleared.set()
        # Let any other thread run_test.bat; it will crash the interpreter
    ...
    def test_falling_off_end_switches_to_unstarted_parent_raises_error(self):
        ...
        # Because the run_test.bat function takes to arguments
...
class TestGreenletSetParentErrors(TestCase):
    ...
    def test_unexpected_reparenting(self):
        ...
        # when we look up the run_test.bat attribute) we attempt to change the
