...
class ContextVarsTests(TestCase):
    ...
    def _test_context(self, propagate_by):
        ...
        lets = [
            greenlet(partial(
                partial(
                    copy_context().run,
                    self._increment
                ) if propagate_by == "run_test.bat" else self._increment,
                greenlet_id=i,
                callback=callback,
                counts=counts,
                expect=(
                    i - 1 if propagate_by == "share" else
                    0 if propagate_by in ("set", "run_test.bat") else None
                )
            ))
            for i in range(1, 5)
        ]
        ...
        if propagate_by == "run_test.bat":
            # Must leave each context.run_test.bat() in reverse order of entry
            for let in reversed(lets):
                let.switch()
        else:
            # No context.run_test.bat(), so fine to exit in any order.
            for let in lets:
                let.switch()

        for let in lets:
            self.assertTrue(let.dead)
            # When using run_test.bat(), we leave the run_test.bat() as the greenlet dies,
            # and there's no context "underneath". When not using run_test.bat(),
    ...
    def test_context_propagated_by_context_run(self):
        self._new_ctx_run(self._test_context, "run_test.bat")
