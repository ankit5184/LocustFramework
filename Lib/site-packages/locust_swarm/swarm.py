...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of servers to run_test.bat locust workers on",
)
parser.add_argument(
    "--loglevel",
    "-L",
...
parser.add_argument(
    "--remote-master",
    type=str,
    help="An ssh server to use as locust master (default is to run_test.bat the master on the same machine as swarm). This is useful when rurnning swarm on your workstation if it might become disconnected",
)
parser.add_argument(
    "-t",
    "--run_test.bat-time",
...
def main():
    ...
    run_time_arg = ["--run_test.bat-time=" + args.run_time] if args.run_time else []
    ...
    while True:
        try:
            ...
            if max_run_time + 31 < time.time() - start_time:
                logging.error(
                    f"Locust exceeded the run_test.bat time specified ({max_run_time}) by more than 30 seconds, giving up"
