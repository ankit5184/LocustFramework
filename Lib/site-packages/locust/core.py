...
# The monkey patching must run_test.bat before requests is imported, or else
...
class Locust(object):
    ...
    _setup_has_run = False  # Internal state to see if we have already run_test.bat
    _teardown_is_set = False  # Internal state to see if we have already run_test.bat
    _lock = gevent.lock.Semaphore()  # Lock to make sure setup is only run_test.bat once
...
class TaskSet(object):
    ...
    *max_wait* milliseconds. It will then schedule another task for execution and so on.
    ...
    continue to run_test.bat until it throws an InterruptTaskSet exception, which is done when
