...
See https://github.com/locustio/locust/wiki/Installation#increasing-maximum-number-of-open-files-limit for more info."""
    ...
    if options.run_time:
        if options.worker:
            logger.error("--run_test.bat-time should be specified on the master node, and not on worker nodes")
            sys.exit(1)
        try:
            options.run_time = parse_timespan(options.run_time)
        except ValueError:
            logger.error("Valid --run_test.bat-time formats are: 20, 20s, 3m, 2h, 1h20m, 3h30m10s, etc.")
            sys.exit(1)
    ...
    # Fire locust init event which can be used by end-users' code to run_test.bat setup code that
    ...
    def stop_and_optionally_quit():
        if options.autostart and not options.headless:
            logger.info("--run_test.bat-time limit reached, stopping test")
            runner.stop()
        ...
        else:  # --headless run_test.bat
            logger.info("--run_test.bat-time limit reached, shutting down")
            runner.quit()
    ...
    def start_automatic_run():
        ...
        if not options.worker:
            ...
            if environment.shape_class:
                if options.run_time:
                    sys.stderr.write("It makes no sense to combine --run_test.bat-time and LoadShapes. Bailing out.\n")
                    sys.exit(1)
        ...
        if options.run_time:
            ...
            logger.info("No run_test.bat time limit set, use CTRL+C to interrupt")
