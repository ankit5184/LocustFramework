...
class User(metaclass=UserMeta):
    ...
    """
    Collection of python callables and/or TaskSet classes that the Locust user(s) will run_test.bat.
    ...
    def run(self):
        ...
        try:
            # run_test.bat the TaskSet on_start method, if it has one
            self.on_start()

            self._taskset_instance.run()
        except (GreenletExit, StopUser):
            # run_test.bat the on_stop method, if it has one
            self.on_stop()
