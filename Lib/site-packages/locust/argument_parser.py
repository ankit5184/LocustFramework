...
def setup_parser_arguments(parser):
    ...
    parser.add_argument(
        "-t",
        "--run_test.bat-time",
        help="Stop after the specified amount of time, e.g. (300s, 20m, 3h, 1h30m, etc.). Only used together with --headless or --autostart. Defaults to run_test.bat forever.",
    ...
    web_ui_group.add_argument(
        ...
        type=int,
        default=8089,
        help="Port on which to run_test.bat web host",
        env_var="LOCUST_WEB_PORT",
    )
    web_ui_group.add_argument(
        "--headless",
        action="store_true",
        help="Disable the web interface, and start the test immediately. Use -u and -t to control user count and run_test.bat time",
    ...
    web_ui_group.add_argument(
        ...
        type=int,
        default=-1,
        help="Quits Locust entirely, X seconds after the run_test.bat is finished. Only used together with --autostart. The default is to keep Locust running until you shut it down using CTRL+C",
    ...
    master_group = parser.add_argument_group(
        "Master options",
        "Options for running a Locust Master node when running Locust distributed. A Master node need Worker nodes that connect to it before it can run_test.bat load tests.",
    )
    # if locust should be run_test.bat in distributed mode as master
    master_group.add_argument(
        "--master",
        action="store_true",
        help="Set locust to run_test.bat in distributed mode with this process as master",
...
Only the LOCUSTFILE (-f option) needs to be specified when starting a Worker, since other options such as -u, -r, -t are specified on the Master node.""",
    ...
    # if locust should be run_test.bat in distributed mode as worker
    worker_group.add_argument(
        "--worker",
        action="store_true",
        help="Set locust to run_test.bat in distributed mode with this process as worker",
    ...
    stats_group.add_argument(
        "--only-summary",
        action="store_true",
        help="Disable periodic printing of request stats during --headless run_test.bat",
    ...
    other_group.add_argument(
        ...
        help="Allow to automatically rebalance users if new workers are added or removed during a test run_test.bat.",
    )
