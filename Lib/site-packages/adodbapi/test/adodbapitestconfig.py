# Configure this to _YOUR_ environment in order to run_test.bat the testcases.
...
if "--help" in sys.argv:
    ...
    --package - create a temporary test package, run_test.bat 2to3 if needed.
    --all - run_test.bat all possible tests
...
# function to clean up the temporary folder -- calling program must run_test.bat this function before exit.
cleanup = setuptestframework.getcleanupfunction()
try:
    import adodbapi  # will (hopefully) be imported using the "pth" discovered above
except SyntaxError:
    print(
        '\n* * * Are you trying to run_test.bat Python2 code using Python3? Re-run_test.bat this test using the "--package" switch.'
    )
    sys.exit(11)
try:
