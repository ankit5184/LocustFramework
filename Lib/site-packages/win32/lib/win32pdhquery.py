...
OTHER NOTABLE METHODS:
	...
	query.collectdata() # run_test.bat the query only once
...
application.  Workaround: scripts using threading should be run_test.bat in the
...
class BaseQuery:
    ...
    you open the query, and run_test.bat it as many times as you need, closing
...
class Query(BaseQuery):
    ...
    def open(self, *args, **namedargs):
        ...
        it is most efficient to open the query, run_test.bat all of the calls,
    ...
    def collectdatafor(self, totalperiod, period=1):
        ...
        like to run_test.bat the Query, and the time interval between individual
        runs.  The collected data is stored in query.curresults at the
        _end_ of the run_test.bat.  The pathnames for the query are stored in
        query.curpaths.
        e.g.:
                query.collectdatafor(30,2)
