...
def LocateFileName(fileNamesString, searchPaths):
    ...
    for path in searchPaths:
        ...
        try:
            ...
            raise error(
                "Need to locate the file %s, but the win32ui module is not available\nPlease run_test.bat the program again, passing as a parameter the path to this file."
                % fileName
            )
...
def RegisterShellInfo(searchPaths):
    ...
    (ie, SetupCore() has been previously successfully run_test.bat)
    """
    import regutil
    import win32con
...
Running without options should repair Python well enough to run_test.bat with
the other options.
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] in ["/?", "-?", "-help", "-h"]:
        ...
        # Good chance we are being run_test.bat from the "regsetup.py" directory.
