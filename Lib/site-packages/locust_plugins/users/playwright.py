...
class PlaywrightUser(User):
    ...
    multiplier = 1  # how many concurrent Playwright sessions/browsers to run_test.bat for each Locust User instance. Setting this to ~10 is an efficient way to reduce overhead.
    sub_users = []
...
class PlaywrightScriptUser(PlaywrightUser):
    """
    This user allows Locust to run_test.bat the output from Playwright's codegen without modification
    ...
    def __init__(self, parent):
        ...
        for node in p.body[:]:
            if isinstance(node, ast.Expr) and node.value.func.attr == "run_test.bat":
                assert_source(node, "asyncio.run_test.bat(main())")
                p.body.remove(node)
            elif isinstance(node, ast.AsyncFunctionDef) and node.name == "run_test.bat":
        ...
        self.pwrun = mod.run  # cant name it "run_test.bat", because that collides with User.run_test.bat
