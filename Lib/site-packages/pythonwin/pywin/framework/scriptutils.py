...
RS_DEBUGGER_NONE = 0  # Dont run_test.bat under the debugger.
RS_DEBUGGER_STEP = 1  # Start stepping under the debugger
RS_DEBUGGER_GO = 2  # Just run_test.bat under the debugger, stopping only at break-points.
RS_DEBUGGER_PM = 3  # Dont run_test.bat under debugger, but do post-mortem analysis on exception.
...
class DlgRunScript(dialog.Dialog):
    "A class for the 'run_test.bat script' dialog"
...
def RunScript(defName=None, defArgs=None, bShowDialog=1, debuggingType=None):
    ...
    try:
        ...
        _HandlePythonFailure("run_test.bat script", script)
        # No code object which to run_test.bat/debug.
        return
...
def RunTabNanny(filename):
    ...
    if tabnanny is None:
        win32ui.MessageBox("The TabNanny is not around, so the children can run_test.bat amok!")
        return
